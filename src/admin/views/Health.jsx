...
const CHECKS = [
  ...
  [
    ...
    "Login attempts are logged here (best-effort until it exists)",
  ],
  [
    "shops_is_deleted",
  ...
  [
    "shops_locality",
    "shops.locality column",
    "Enables the “Top performing area” card (decision 1)",
  ],
  [
  ...
  [
    "shop_ratings",
    "shop_ratings table",
    "Ratings & Reviews moderation queue + distribution",
  ],
  [
    "admin_analytics_rpc",
    "admin_analytics() RPC",
      ...
      <div className="ap-view-head">
        <div>
          ...
          <p className="ap-view-sub">
            Migration status &amp; known data-integrity notes
          </p>
        </div>
          ...
          <ul className="ap-checklist">
            {CHECKS.map(([key, label, desc]) => (
              <li key={key} className={data?.[key] ? "is-ok" : "is-missing"}>
                ...
                <span className="ap-check-state">
                  {data?.[key] ? "installed" : "missing"}
                </span>
              </li>
            ))}
          </ul>
